...
id: python
slug: python
title: Analyze JSON data using Python
description: "Alljson allows you to analyze the JSON content using Python language"
---

# Python

[Analyze JSON data using Python (https://alljson.com/open-python)](https://alljson.com/open-python)

Alljson allows you to analyze the JSON content using Python language.

### Example: user interface and Jsonata query

```python
import pyjsonata
from alljson import LEFT_EDITOR_TO_STRING
...
# HTTP request
x = requests.get('https://alljson.com/examples/exampleGetPlanets')
# analyzing by using pyjsonata library
...
Additionally, there are non-blocking Alljson API functions available for sequential execution mode, ensuring operation without blocking the GUI:

import { Func, Param } from '@site/src/components/Script';

- <Func>import alljson</Func> or <Func>from alljson import **FUNCTION**</Func> - to use
  the Alljson API, you need to import the alljson module.
- <Func>alljson.CLEAR_CONSOLE</Func>() - clear console
- <Func>alljson.CONSOLE</Func>(<Param>text or object</Param>) - print text or
  object on console box
- <Func>alljson.ERROR</Func>(<Param>error message</Param>) - print error message
  on console box
- <Func>alljson.LEFT_EDITOR_TO_STRING</Func>() - return text from the left editor
- <Func>alljson.RIGHT_EDITOR_TO_STRING</Func>() - return text from the right
  editor
- <Func>alljson.STRING_TO_LEFT_EDITOR</Func>(<Param>text</Param>) - write text to
  the left editor
- <Func>alljson.STRING_TO_RIGHT_EDITOR</Func>(<Param>text</Param>) - write text to
  the right editor
- <Func>alljson.SET_CURRENT_THEME</Func>(<Param>"light"|"dark"</Param>) - change
  current user interface theme
- <Func>alljson.GET_CURRENT_THEME</Func>() - get current user interface theme
  name. Return: "light" or "dark"
- <Func>alljson.CLEAR_OUTPUT</Func>() - clear python output: output box with the
  DOM element ID = GET_OUTPUT_HTML_ELEMENT_ID()
- <Func>alljson.DISPLAY_IN_OUTPUT</Func>(<Param>*values</Param>, <Param>
    target=None
...
- <Func>alljson.GET_OUTPUT_HTML_ELEMENT_ID</Func>() - retrieve the ID of the
  output DOM element to interact with the user interface using the JavaScript
  API in Python. See example below:

```python
from js import document
from alljson import GET_OUTPUT_HTML_ELEMENT_ID
...
- <Func>alljson.INSTALL_PYTHON_MODULES</Func>(<Param>list or item</Param>) -
...
```python
from alljson import INSTALL_PYTHON_MODULES
...
- <Func>alljson.LOAD_SCRIPTS_OR_STYLES</Func>(<Param>list or item</Param>) -
  Loading scripts or styles from external sources, or executing a script from
  text. See examples below:

```python
from alljson import LOAD_SCRIPTS_OR_STYLES
